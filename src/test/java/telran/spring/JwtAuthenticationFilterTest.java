...
static String jwt;
	...
	void authenticationErrorTest() throws Exception {
		mockMvc.perform(get("http://localhost:8800/kuku"))
	...
	void loginTest() throws Exception {
		String loginResponseJson = mockMvc.perform(post("http://localhost:8800/login").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(loginData)))
	...
	void authenticationNormalTest() throws Exception {
		...
		mockMvc.perform(get("http://localhost:8800/kuku").header("Authorization", "Bearer " + jwt))
